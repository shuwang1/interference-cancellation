...
LP = L*P
M = 60            %% the columns of the blind spreading matrix. 
...
for n =  1:n_Len
    ...
    for m = 1:length(SNR)
        ...
        for t = M-G+1 : TotalTrails
            ...
            f_BMMSE = (C_SNR*sigma+S_BMMSE'*S_BMMSE)\(S_BMMSE'*r(:,t));
            ...
            [ Qg, Rg ] = qr( S_G );
            S_qr = Qg' * [ S_MLS r(:,t) ];
            [U_MLS, D_MLS, V_MLS] = svd( S_qr(G+1:LP,G+1:M+1), 0 );
...
switch simulationConfig
    ...
    case 2
        figure(20)
        semilogy(SNR, BER_MF(:,2),'-+',SNR, BER_MF(:,3),'-x', SNR, BER_DD(:,2),'-o', SNR, BER_LS(:,2),'-p', SNR, BER_LS(:,3), SNR, BER_TLS(:,2),'-p', SNR, BER_TLS(:,3), SNR, BER_MLS(:,2),'-p', SNR, BER_MLS(:,3),'-h','LineWidth',2,'MarkerSize',5)
        ...
        ylabel('Bit-Error-Rate BER','FontSize',14,'FontWeight','bold');
        legend('MF with A_2/A_1=1.0','MF with A_2/A_1=10','DD with A_2/A_1=1.0','LS with A_2/A_1=1.0','LS with A_2/A_1=10','TLS with A_2/A_1=1.0','TLS with A_2/A_1=10','MLS with A_2/A_1=1.0','MLS with A_2/A_1=10');
