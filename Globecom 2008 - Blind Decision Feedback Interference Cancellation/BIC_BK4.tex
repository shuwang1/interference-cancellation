...
It can be proven that the LS-IC in (\ref{b_LS_IC}) actually is a
blind mixed LS/TLS (MLS) decorrelating detector
\begin{equation}
\begin{array}{rcl}
\hat{\bb}_{1}&=&\mbox{sign}\left\{\bG\bbf_{\rm MLS}\right\}
\end{array}
\end{equation}
\noindent with respect to
\begin{equation}\hspace{-0.20in}
\begin{array}{l}
\left[\matrix{\bcS_{2\rm MLS}\cr\bbf_{\rm
MLS}}\right]=\matrix{\mbox{arg}\min\limits_{\hat{\bcS}_2,\
\bx}\left\|\left[\matrix{\bcS_2\cr\br}\right]-\left[\matrix{\hat{\bcS}_2\cr\left[\bcS_1\
 \hat{\bcS}_2\right]\bx}\right]\right\|_{2}^{2} }
\end{array}\label{MLSProb}
\end{equation}
\noindent and MMSE-IC is the same to the blind MMSE detector
\begin{equation}\hspace{0.0in}
\begin{array}{rcl}
\hat{\bb}_1&=&\mbox{sign}\left\{\bG\bbf_{\rm MMSE}\right\}
\end{array}
\end{equation}
\noindent with respect to
\begin{equation}
