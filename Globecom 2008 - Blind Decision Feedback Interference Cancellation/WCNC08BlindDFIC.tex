...
criteria are proposed and analyzed for solving the near-far
problem. Compared with most existing blind multiuser receivers,
the proposed framework and schemes require a small number of
previously received signals and no subspace separation or sequence
estimation. The incurred detection complexity and delay can
therefore be low. The proposed schemes can further be adaptively
and iteratively implemented for improving detection performance.
Theoretical analysis and the comparison with existing multiuser
receivers as well as computer simulations are provided to
