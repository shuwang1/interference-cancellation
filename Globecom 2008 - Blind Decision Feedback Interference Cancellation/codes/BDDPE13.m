...
for n = 1:length(A21)
    for m = 1:length(SNR)
        ...
        while trial <= TotalTrails
            ...
            B1 = [ [1/A(1,1); zeros(K-1,1)] ( (rand(K, M-1) > 0.5)*2 -1 ) ];
            B2 = [ [1/A(1,1); zeros(K-1,1)] ( (rand(K, M-1) > 0.5)*2 -1 ) ];
            d1 = pinv(B1)*b;
            d2 = pinv(B2)*b;
            
            
            while abs( B1(1,2:M)*d1(2:M,1)-B2(1,2:M)*d2(2:M,1) ) < 0.01
                B1 = [ [1/A(1,1); zeros(K-1,1)] ( (rand(K, M-1) > 0.5)*2 -1 ) ];
                B2 = [ [1/A(1,1); zeros(K-1,1)] ( (rand(K, M-1) > 0.5)*2 -1 ) ]; 
                d1 = pinv(B1)*b;
                d2 = pinv(B2)*b;                
            ...
            d1_LS = ( pinv(S_B1'*S_B1)*S_B1' )*r;
            d2_LS = ( pinv(S_B2'*S_B2)*S_B2' )*r;
            ...
            b_LS = sign( [1/mean(A1_LS(1:trial,1)) B1(1,2:M)]*d1_LS + [1/mean(A1_LS(1:trial,1)) B2(1,2:M)]*d2_LS );
            ...
            b_TLS = sign( [1/mean(A1_TLS(1:trial,1)) B1(1,2:M)]*d1_TLS + [1/mean(A1_TLS(1:trial,1)) B2(1,2:M)]*d2_TLS );
            ...
            b_MLS = sign( [1/mean(A1_MLS(1:trial,1)) B1(1,2:M)]*d1_MLS + [1/mean(A1_MLS(1:trial,1)) B2(1,2:M)]*d2_MLS );

            %=================================================%
                
            b_DD = w_DD' * r(:,1);
            A1_DD(trial,1) = abs(b_DD);
            P_DD(trial,1) = b_DD^2;
            
            b_MF = w_MF' * r(:,1);
            A1_MF(trial,1) = abs(b_MF);            
            P_MF(trial,1) = b_MF^2;
        ...
        A_LS_Mean(m,n) = mean(A1_LS);
        A_TLS_Mean(m,n) = mean(A1_TLS);
        A_MLS_Mean(m,n) = mean(A1_MLS);
        A_DD_Mean(m,n) = mean(A1_DD);
        A_MF_Mean(m,n) = mean(A1_MF);        
        
        A_LS_Std(m,n) = std(A1_LS);
        A_TLS_Std(m,n) = std(A1_TLS);
        A_MLS_Std(m,n) = std(A1_MLS);        
        A_DD_Std(m,n) = std(A1_DD);
        A_MF_Std(m,n) = std(A1_MF);        
...
figure(5)
semilogy(SNR, P_MF_Std(:,1),'-s', SNR, P_DD_Std(:,1),'-o',SNR, P_LS_Std(:,1),'-^', SNR, P_TLS_Std(:,1),'-v', SNR, P_MLS_Std(:,1),'-d')
grid;
...
figure(6)
semilogy(SNR, P_MF_Std(:,3),'-s', SNR, P_DD_Std(:,3),'-o',SNR, P_LS_Std(:,3),'-^', SNR, P_TLS_Std(:,3),'-v', SNR, P_MLS_Std(:,3),'-d')
grid;
...
p=1
figure(31)
plot(SNR,ones(size(SNR)),'-',SNR, A_MF_Mean(:,p),'--',SNR, A_DD_Mean(:,p),':', SNR, A_LS_Mean(:,p),'-s',SNR, A_TLS_Mean(:,p),'-d',SNR, A_MLS_Mean(:,p),'-o','LineWidth',1,'MarkerSize',6)
grid;
title('The Perofmance of The Power Estimation Schemes with A_2/A_1=0.1');
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('Mean of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('The orginal power','MF with A_2/A_1=0.1','DD with A_2/A_1=0.1','LS with A_2/A_1=0.1','TLS with A_2/A_1=0.1','MLS with A_2/A_1=0.1');

figure(32)
plot(SNR, A_MF_Std(:,p),'--',  SNR, A_DD_Std(:,p),'-', SNR, A_LS_Std(:,p),'-s',SNR, A_TLS_Std(:,p),'-d', SNR, A_MLS_Std(:,p),'-o','LineWidth',1,'MarkerSize',6)
grid;
title('The Perofmance of The Power Estimation Schemes with A_2/A_1=0.1');
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('STD of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('MF with A_2/A_1=0.1','DD with A_2/A_1=0.1','LS with A_2/A_1=0.1','TLS with A_2/A_1=0.1','MLS with A_2/A_1=0.1');


figure(33)
semilogy(SNR, BER_MF(:,p),'--', SNR, BER_DD(:,p),'-', SNR, BER_LS(:,p),'-s', SNR, BER_TLS(:,p),'-d', SNR, BER_MLS(:,p),'-o','LineWidth',1,'MarkerSize',6)
ylim([1.0/TotalTrails 0.55])
xlim([min(SNR) max(SNR)])
grid;
title('BER of Different Multiuser Detection Algorithms');
...
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('Mean of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('The orginal power','MF with A_2/A_1=1.0','DD with A_2/A_1=1.0','LS with A_2/A_1=1.0','TLS with A_2/A_1=1.0','MLS with A_2/A_1=1.0');
...
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('STD of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('MF with A_2/A_1=1.0','DD with A_2/A_1=1.0','LS with A_2/A_1=1.0','TLS with A_2/A_1=1.0','MLS with A_2/A_1=1.0');
...
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('Mean of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('The orginal power','MF with A_2/A_1=10','DD with A_2/A_1=10','LS with A_2/A_1=10','TLS with A_2/A_1=10','MLS with A_2/A_1=10');
...
xlabel('SNR (dB)','FontWeight','bold','FontSize',14);
ylabel('STD of the estimation of P_1','FontWeight','bold','FontSize',14);
legend('MF with A_2/A_1=10','DD with A_2/A_1=10','LS with A_2/A_1=10','TLS with A_2/A_1=10','MLS with A_2/A_1=10');
