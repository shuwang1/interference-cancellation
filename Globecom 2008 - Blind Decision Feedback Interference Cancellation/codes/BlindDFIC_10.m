...
LP = L*P
M =  30            %% the columns of the blind spreading matrix. 
...
tao = [0; ceil( rand(K0-1, 1)*floor((L-1)/2) )*2 ]    %% this is the propagation delay vector used 
...
switch  IsAsync 
    ...
    case 1
        ...
        for k = 2:K0
            ...
            a(P*(k-1)+1:P*k+1, 1) = a0(k).*ones(P+1,1);
            for p = 1:P
                S( (p*L-L+1):(p*L), (P+(k-2)*(P+1)+p):(P+(k-2)*(P+1)+p+1) ) = [sk_ s_k];
            end
        end
...
for n =  1:n_Len
    ...
    switch simulationConfig
        ...
        case 5
            ...
            C_SNR = eye(M)*(M*2-(P+1)*K0);
            C_SNR(1,1) = 1;
            R_S = S(1:LP,1:(P*K0+K0-1))'*S(1:LP,1:(P*K0+K0-1));
            R_S_ = R_S\eye(P*K0+K0-1)
            %%--------------------------------------------------------
            %% Construct MF receiver and several conventional MUDs.
            %%--------------------------------------------------------
            w_MF  = S(1:LP,1);
    ...
    for m = 1:length(SNR)
        ...
        r(1:LP,1:M-G+1) = S(1:LP,1:K)*A(1:K,1:K)*B(1:K,G+1:M+1);% + sigma*randn(P*L,M-G+1);
        ...
        for t = M-G+1 : TotalTrails
            ...
            S_LS(1:LP,G+1:M) = [ S_LS(1:LP,(G+2):M) r(1:LP, t-1) ];
            switch amplitudeFiltering
                case -2
            ...
            f_BMMSE = pinv(C_SNR*sigma+S_BMMSE'*S_BMMSE)*(S_BMMSE'*r(1:LP,t));
            ...
            b_MF(:, t) = w_MF' * r(1:LP,t);
            a_MF(:, t) = abs( b_MF(:, t) );
            
            b_DD(:, t) = w_DD' * r(1:LP,t);
            a_DD(:, t) = abs( b_DD(:, t) );
...
switch simulationConfig
    ...
    case 2
        ...
        ylabel('BER','FontSize',24,'FontWeight','bold');
        legend('SU-MF with A_2/A_1=1.0','SU-MF with A_2/A_1=10','DD with A_2/A_1=1.0','MVU with A_2/A_1=1.0','MUV with A_2/A_1=10','MMSE with A_2/A_1=1.0','MMSE with A_2/A_1=10');
