...
for n = 1:length(A21)
    for m = 1:length(SNR)
        ...
        while trial <= TotalTrails
            ...
            B1 = [ [1/A(1,1);0] ( (rand(2,1) > 0.5)*2 -1 ) ];
            B2 = [ [1/A(1,1);0] ( (rand(2,1) > 0.5)*2 -1 ) ];
            ...
            [Us1, Ss1, Vs1]=svd([P*S_B1(:,2:K) P*r]);
            s_svd1=sort(diag(Ss1));

            d1_TLS(2:K,1) = ( inv( (P*S_B1(:,2:K))'*(P*S_B1(:,2:K)) - (s_svd1(1,1)^2)*eye(K-1) )*(P*S_B1(:,2:K))' )*P*r;
            d1_TLS(1,1) = s1'*( r-S_B1(:,2:K)*d1_TLS(2:K,1) );
            
            [Us2, Ss2, Vs2]=svd([P*S_B2(:,2:K) P*r]);
            s_svd2=sort(diag(Ss2));
            
            d2_TLS(2:K,1) = ( inv( (P*S_B2(:,2:K))'*(P*S_B2(:,2:K)) - (s_svd2(1,1)^2)*eye(K-1) )*(P*S_B2(:,2:K))' )*P*r;
            d2_TLS(1,1) = s1'*( r-S_B2(:,2:K)*d2_TLS(2:K,1) );
            ...
            A1_LS(trial,1)  = abs( delta_d_LS(1,1)/( d1_LS(2,1)*B1(1,2)-d2_LS(2,1)*B2(1,2) ) );
            A1_TLS(trial,1) = abs( delta_d_TLS(1,1)/( d1_TLS(2,1)*B1(1,2)-d2_TLS(2,1)*B2(1,2) ) );
                
            P1_LS(trial,1)  = A1_LS(trial,1)^2;
            P1_TLS(trial,1) = A1_TLS(trial,1)^2;
                
            b_LS = sign( [1/A1_LS(trial,1) B1(1,2)]*d1_LS + [1/A1_LS(trial,1) B2(1,2)]*d2_LS );
            b_TLS = sign( [1/A1_TLS(trial,1) B1(1,2)]*d1_TLS + [1/A1_TLS(trial,1) B2(1,2)]*d2_TLS );
