...
for n = 1:length(A21)
    for m = 1:length(SNR)
        ...
        while trial <= TotalTrails
            ...
            B1 = [ [1/A(1,1);0] ( (rand(2,2) > 0.5)*2 -1 ) ];
            B2 = [ [1/A(1,1);0] ( (rand(2,2) > 0.5)*2 -1 ) ];
            ...
            [Us1, Ss1, Vs1]=svd([P*S_B1(:,2:M) P*r]);
            s_svd1=sort(diag(Ss1));

            d1_TLS(2:M,1) = ( inv( (P*S_B1(:,2:M))'*(P*S_B1(:,2:M)) - (s_svd1(1,1)^2)*eye(M-1) )*(P*S_B1(:,2:M))' )*P*r;
            d1_TLS(1,1) = s1'*( r-S_B1(:,2:M)*d1_TLS(2:M,1) );
            
            [Us2, Ss2, Vs2]=svd([P*S_B2(:,2:M) P*r]);
            s_svd2=sort(diag(Ss2));
            
            d2_TLS(2:M,1) = ( inv( (P*S_B2(:,2:M))'*(P*S_B2(:,2:M)) - (s_svd2(1,1)^2)*eye(M-1) )*(P*S_B2(:,2:M))' )*P*r;
            d2_TLS(1,1) = s1'*( r-S_B2(:,2:M)*d2_TLS(2:M,1) );
