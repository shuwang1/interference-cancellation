...
for n = 1:length(A21)
    for m = 1:length(SNR)
        ...
        while trial <= TotalTrails
            ...
            %================================================%            
            [Us1, Ss1, Vs1]=svd([S_B1(:,2:M) r]);
            [s_svd1 index]=sort(diag(Ss1));
            Us1 = Us1(:,index);
            Vs1 = Vs1(:,index)
            
            %d1_TLS(2:M,1) = -Vs1(1:M-1,1)/Vs1(M,1)
            d1_TLS(2:M,1) = ( inv( S_B1(:,2:M)'*S_B1(:,2:M) - (s_svd1(1,1)^2)*eye(M-1) )*S_B1(:,2:M)' )*r
            
            
            pause
            
            D_S_r = s_svd1(1,1)*Us1(:,1)*Vs1(:,1)';
            temp=[S_B1(:,2:M) r] - D_S_r;
            %S_B1(:,2:M)=temp(:,1:M-1);
            r1=temp(:,M);

            d1_TLS(1,1) = s1'*(r1-S_B1(:,2:M)*d1_TLS(2:M,1));
            %================================================%
            [Us2, Ss2, Vs2]=svd([S_B2(:,2:M) r]);
            [s_svd2 index]=sort(diag(Ss2));
            Us2 = Us2(:,index);
            Vs2 = Vs2(:,index);
            
            
            d2_TLS(2:M,1) = ( inv( S_B2(:,2:M)'*S_B2(:,2:M) - (s_svd2(1,1)^2)*eye(M-1) )*S_B2(:,2:M)' )*r;

            D_S_r = s_svd2(1,1)*Us2(:,1)*Vs2(:,1)';
            temp=[S_B2(:,2:M) r] - D_S_r;
            %S_B2(:,2:M)=temp(:,1:M-1);
            r2=temp(:,M);

            d2_TLS(1,1) = s1'*(r2-S_B2(:,2:M)*d2_TLS(2:M,1));
            %=================================================%
