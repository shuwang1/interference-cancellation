...
LP = L*P
M = 40          %% the columns of the blind spreading matrix. 
                    %% Theorectially, the large number, the better performance.  
G = 4             %% the group size. the default is 1. 
IsAsync = 0
...
switch simulationConfig
    ...
    case 2
        ...
        figure(25)
        subplot(1,2,1);
        semilogy(SNR, BER_MF(:,1),'-x', SNR, BER_MF(:,2),'-+',SNR, BER_DD(:,2),'--', SNR, BER_LS(:,1),'-d', SNR, BER_LS(:,2),'-s',  SNR, BER_TLS(:,1),'-<', SNR, BER_TLS(:,2),'->', SNR, BER_MLS(:,1),'-v', SNR, BER_MLS(:,2),'-o', 'LineWidth',4,'MarkerSize',10)
        ylim([1.0/TotalTrails 0.55])
        xlim([min(SNR) max(SNR)])
        grid;
        %title('BER of Different Multiuser Detection Algorithms');
        xlabel('E_b/N_o (dB)','FontSize',24,'FontWeight','bold');
        ylabel('BER','FontSize',24,'FontWeight','bold');
        legend('MF with A_2/A_1=0.1','MF with A_2/A_1=1.0','DD with A_2/A_1=1.0','LS with A_2/A_1=0.1','LS with A_2/A_1=1.0','TLS with A_2/A_1=0.1','TLS with A_2/A_1=1.0','MLS with A_2/A_1=0.1','MLS with A_2/A_1=1.0');
        subplot(1,2,2)
        semilogy(SNR, BER_MF(:,2),'-+',SNR, BER_MF(:,3),'-x', SNR, BER_DD(:,2),'--',  SNR, BER_LS(:,2),'-s', SNR, BER_LS(:,3), '-d', SNR, BER_TLS(:,2),'->', SNR, BER_TLS(:,3),'-<',  SNR, BER_MLS(:,2),'-o', SNR, BER_MLS(:,3),'-v','LineWidth',4,'MarkerSize',10)
        ylim([1.0/TotalTrails 0.55])
        xlim([min(SNR) max(SNR)])
        grid;
        %title('BER of Different Multiuser Detection Algorithms');
        xlabel('E_b/N_o (dB)','FontSize',24,'FontWeight','bold');
        ylabel('BER','FontSize',24,'FontWeight','bold');
        legend('MF with A_2/A_1=0.1','MF with A_2/A_1=10','DD with A_2/A_1=1.0','LS with A_2/A_1=1.0','LS with A_2/A_1=10','TLS with A_2/A_1=1.0','TLS with A_2/A_1=10','MLS with A_2/A_1=1.0','MLS with A_2/A_1=10');
